...
module.exports = function getModelsMapForPopulate(model, docs, options) {
  ...
  const map = [];
  const modelNameFromQuery =
    (options.model && options.model.modelName) || options.model;
  let schema;
  let refPath;
  let modelNames;
  const available = {};
  ...
  if (
    options._localModel != null &&
    options._localModel.schema.nested[options.path]
  ) {
    return [];
  }
  ...
  // document.populate([{ path: keyString, strictPopulate: false }]); //Manually added by Rajan
  allSchemaTypes = Array.isArray(allSchemaTypes)
    ? allSchemaTypes
    : [allSchemaTypes].filter((v) => v != null);
  ...
  for (let i = 0; i < len; i++) {
    ...
    const docSchema =
      doc != null && doc.$__ != null ? doc.$__schema : modelSchema;
    ...
    if (isUnderneathDocArray && get(options, "options.sort") != null) {
      return new MongooseError(
        "Cannot populate with `sort` on path " +
          options.path +
    ...
    if ("justOne" in options && options.justOne !== void 0) {
      ...
      if (
        options.path.endsWith("." + schema.path) ||
        options.path === schema.path
      ) {
    ...
    // Get local fields
    const ret = _getLocalFieldValues(
      doc,
      localField,
      model,
      options,
    ...
    if (modelNameFromQuery) {
      ...
      if (typeof refPath === "function") {
        const subdocPath = options.path.slice(
          0,
          options.path.length - schema.path.length - 1
        );
        ...
        for (const subdoc of subdocsBeingPopulated) {
          ...
          modelNamesFromRefPath(
            refPath,
            doc,
            options.path,
            modelSchema,
            options._queryProjection
          ).forEach((name) => modelNames.add(name));
        }
        modelNames = Array.from(modelNames);
      } else {
        modelNames = modelNamesFromRefPath(
          refPath,
          doc,
          options.path,
          modelSchema,
      ...
      if (
        !schema &&
        discriminatorKey &&
        (discriminatorValue = utils.getValue(discriminatorKey, doc))
      ) {
        // `modelNameForFind` is the discriminator value, so we might need
        // find the discriminated model name
        const discriminatorModel =
          getDiscriminatorByValue(model.discriminators, discriminatorValue) ||
          model;
        ...
        schemaForCurrentDoc = modelForCurrentDoc.schema._getSchema(
          options.path
        );
      ...
      if (schemaForCurrentDoc != null) {
        justOne =
          !schemaForCurrentDoc.$isMongooseArray &&
          !schemaForCurrentDoc._arrayPath;
      }
      ...
      if ((ref = get(schemaForCurrentDoc, "options.ref")) != null) {
        if (
          ...
          const subdocPath = options.path.slice(
            0,
            options.path.length - schemaForCurrentDoc.path.length - 1
          );
          const vals = mpath.get(subdocPath, doc, lookupLocalFields);
          const subdocsBeingPopulated = Array.isArray(vals)
            ? utils.array.flatten(vals)
...
function _virtualPopulate(model, docs, options, _virtualRes) {
  ...
  for (const doc of docs) {
    ...
    if (typeof virtual.options.localField === "function") {
      ...
      localField = virtual.options.localField.map(
        (field) => virtualPrefix + field
      );
    } else {
    ...
    if (
      virtual.options.perDocumentLimit != null &&
      !options.hasOwnProperty("perDocumentLimit")
    ) {
    ...
    if (!localField || !foreignField) {
      return new MongooseError(
        "If you are populating a virtual, you must set the " +
    ...
    if (
      ...
      Array.isArray(foreignField) &&
      localField.length === foreignField.length
    ) {
      match = Object.assign({}, match);
      for (let i = 1; i < localField.length; ++i) {
        match[foreignField[i]] = convertTo_id(
          mpath.get(localField[i], doc, lookupLocalFields),
          model.schema
        );
...
function _getLocalFieldValues(
  ...
  const localFieldPath =
    localFieldPathType === "real"
      ? model.schema.path(localField)
      : localFieldPathType.schema;
  const localFieldGetters =
    localFieldPath && localFieldPath.getters ? localFieldPath.getters : [];

  const _populateOptions = get(options, "options", {});

  const getters =
    "getters" in _populateOptions
  ...
  if (localFieldGetters.length > 0 && getters) {
    ...
    if (Array.isArray(localFieldValue)) {
      const localFieldHydratedValue = utils.getValue(
        localField.split(".").slice(0, -1),
        hydratedDoc
      );
      return localFieldValue.map((localFieldArrVal, localFieldArrIndex) =>
        localFieldPath.applyGetters(
          localFieldArrVal,
          localFieldHydratedValue[localFieldArrIndex]
        )
      );
    } else {
...
function convertTo_id(val, schema) {
  ...
  if (
    val != null &&
    val._id != null &&
    (schema == null || !schema.$isSchemaMap)
  ) {
    return val._id;
  }
...
function _findRefPathForDiscriminators(
  doc,
  modelSchema,
  data,
  options,
  ...
  for (let i = 0; i < pieces.length; ++i) {
    ...
    if (
      ...
      const discriminatorKey =
        schematype.caster.schema.options.discriminatorKey;
      modelNames = [];
      for (const subdoc of subdocs) {
        const discriminatorName = utils.getValue(discriminatorKey, subdoc);
        const discriminator =
          schematype.caster.discriminators[discriminatorName];
        ...
        if (_path == null || _path.options.refPath == null) {
          const docValue = utils.getValue(
            data.localField.substr(cur.length + 1),
            subdoc
          );
          ret.forEach((v, i) => {
            if (v === docValue) {
